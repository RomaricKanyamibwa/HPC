...
int main(int argc,char* argv[])
{
    ...
    int prof;
    /* Image resultat */
    unsigned char *pima_loc, *pima,*ima;
    ...
    if(h%p==0)
        hprime=h/p;//longuer de la bande
    else
    {
        fprintf( stderr, "Erreur h mod NP!=0(%d mod %d=%d)!!\n",h,p,h%p);
        return 0;
    }
    ...
    if(my_rank==0)
    {
        ...
        ima=pima = (unsigned char *)malloc( w*h*sizeof(unsigned char));
        pima_loc = (unsigned char *)malloc( w*hprime*sizeof(unsigned char));

        if( pima == NULL || pima_loc == NULL) {
        ...
        {
            source=i;
            MPI_Recv(pima_loc,w*hprime,MPI_UNSIGNED_CHAR,source,tag,MPI_COMM_WORLD,&status);
            memcpy(ima+w*hprime*i,pima_loc,w*hprime);
        }
        fin=MPI_Wtime();
         /* Sauvegarde de la grille dans le fichier resultat "mandel.ras" */
        sauver_rasterfile( "mandel_paral.ras", w, h, ima);
    ...
    {
        printf("Mandelbrot SET HPC #%d\n",my_rank);
        y = ymin_prime;
        ima=pima_loc = (unsigned char *)malloc( w*hprime*sizeof(unsigned char));
        for (i = 0; i < hprime; i++) {
            x = xmin_prime;
            for (j = 0; j < w; j++) {
              // printf("%d\n", xy2color( x, y, prof));
