...
unsigned char* mandel_non_paral()//fonction à executer sequenteillement
{
    unsigned char *pima_loc,*ima;
    /* Variables intermediaires */
    int  i, j;
    double x, y;
    printf("-------------------------------Version Sequentiel-------------------------------");
    /* Allocation memoire du tableau resultat */
    ima = pima_loc = (unsigned char *)malloc( w*h*sizeof(unsigned char));
    if( ima == NULL) {
...
{
  //unsigned char *ima=pima = (unsigned char *)malloc( w*h*sizeof(unsigned char));
  // int nblocs=h/nlines,i;
  unsigned int blockid=0,blockidresp,i;
  MPI_Status status;
  //unsigned char* pima_loc;
  //pima_loc = (unsigned char *)malloc( w*nlines*sizeof(unsigned int));

  if(NP<=1)
  {
      pima=mandel_non_paral();
      return 1;
  }
